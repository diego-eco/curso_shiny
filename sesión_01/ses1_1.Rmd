...
rm(list = ls()) # Limpieamos el environment
...
data("penguins") # Nos muestra "promises" en ambiente. Son objetos aún no cargados en memoria.
```
...
```{r}
penguins %>%
  filter(island == "Torgersen") # Si es string, se pone entre comillas " "
```
...
# Utilizando read.csv 
base_sucia <- read.csv(paste(data,"BIE_BIE20210423011027.csv",sep="/"), 
                    header = F,
                    skip = 2, 
                    nrows = 195,
...
desem_inflac
ggsave(paste0(figures,"/","desem_inflac.png"), bar)
```
